defmodule ExESDBGater.Repl.Console do
  ...
  def start do
    ...
    IO.puts("#{CF.yellow_on_black()}Available streams: #{inspect(EventGenerator.streams())}#{CF.reset()}")
    main_menu()
  end
  ...
  defp main_menu do
    ...
    choice = IO.gets("\n#{CF.bright_white_on_black()}👉 Select an option (1-15): #{CF.reset()}") |> String.trim()
  ...
  defp handle_choice(_), do: IO.puts("#{CF.red_on_black()}❌ Invalid choice. Please try again.#{CF.reset()}")
  ...
  defp create_observer_by_stream do
    ...
    try do
      ...
      IO.puts("#{CF.green_on_black()}✅ Observer '#{observer_name}' created for stream '#{stream}'#{CF.reset()}")
    rescue
  ...
  defp create_observer_by_event_type do
    ...
    EventGenerator.event_types() |> Enum.with_index(1) |> Enum.each(fn {type, i} ->
      IO.puts("  #{i}. #{type}")
    end)
    ...
    try do
      ...
      IO.puts("#{CF.green_on_black()}✅ Observer '#{observer_name}' created for event type '#{event_type}'#{CF.reset()}")
    rescue
      e -> IO.puts("#{CF.red_on_black()}❌ Failed to create observer: #{inspect(e)}#{CF.reset()}")
    end
  end
  ...
  defp create_observer_by_event_payload do
    ...
    try do
      ...
      IO.puts("#{CF.green_on_black()}✅ Observer '#{observer_name}' created for payload pattern #{inspect(pattern)}#{CF.reset()}")
    rescue
      e -> IO.puts("#{CF.red_on_black()}❌ Failed to create observer: #{inspect(e)}#{CF.reset()}")
    end
  end
  ...
  defp list_observers do
    ...
    if observers == [] do
      ...
      IO.puts("#{CF.cyan_on_black()}#{String.pad_trailing("Name", 20)} #{String.pad_trailing("Type", 20)} #{String.pad_trailing("Selector", 30)} Status#{CF.reset()}")
      IO.puts(String.duplicate("-", 80))
      
      observers |> Enum.each(fn {name, {type, selector, pid}} ->
        status = if Process.alive?(pid), do: "#{CF.green_on_black()}🟢 Running#{CF.reset()}", else: "#{CF.red_on_black()}🔴 Stopped#{CF.reset()}"
        IO.puts("#{String.pad_trailing(name, 20)} #{String.pad_trailing(to_string(type), 20)} #{String.pad_trailing(inspect(selector), 30)} #{status}")
      end)
    end
  end
    ...
    case :ets.lookup(@observer_registry, observer_name) do
      ...
      [{^observer_name, {type, selector, pid}}] ->
        if Process.alive?(pid) do
          ...
          IO.puts("#{CF.cyan_on_black()}Type: #{type}, Selector: #{inspect(selector)}#{CF.reset()}")
          IO.puts("#{CF.dim_white_on_black()}Press Ctrl+C to stop monitoring#{CF.reset()}")
    ...
    case :ets.lookup(@observer_registry, observer_name) do
      ...
      [{^observer_name, {_type, _selector, pid}}] ->
        if Process.alive?(pid) do
          ...
          :ets.delete(@observer_registry, observer_name)
          IO.puts("#{CF.yellow_on_black()}⚠️  Observer '#{observer_name}' was already stopped#{CF.reset()}")
        end
    end
  end
  ...
  defp list_producers do
    ...
    IO.puts("#{CF.yellow_on_black()}ℹ️  Producer listing requires extension of the Producer module#{CF.reset()}")
  end
  ...
  defp create_subscription do
    ...
    try do
      ...
      e -> IO.puts("#{CF.red_on_black()}❌ Failed to create subscription: #{inspect(e)}#{CF.reset()}")
    end
  end
    ...
    case API.get_subscriptions(Repl.store()) do
      {:ok, subscriptions} ->
        if subscriptions == [] do
          ...
          subscriptions |> Enum.each(fn sub ->
            IO.puts("  • #{inspect(sub, pretty: true)}")
          end)
        end
      
      {:error, reason} ->
        IO.puts("#{CF.red_on_black()}❌ Failed to get subscriptions: #{inspect(reason)}#{CF.reset()}")
    end
  end
  ...
  defp view_stream_data do
    ...
    case API.get_streams(Repl.store()) do
      {:ok, streams} ->
        ...
        streams |> Enum.with_index(1) |> Enum.each(fn {stream, i} ->
          IO.puts("  #{i}. #{stream}")
        end)
  ...
  defp append_events_manually do
    ...
    case Repl.append(stream, count) do
      {:ok, events, event_count} ->
        IO.puts("#{CF.green_on_black()}✅ Appended #{event_count} events to '#{stream}'#{CF.reset()}")
        IO.puts("#{CF.cyan_on_black()}Latest events:#{CF.reset()}")
        events |> Enum.take(3) |> Enum.each(fn event ->
  ...
  defp get_cluster_status do
    ...
    try do
      ...
      IO.puts("#{CF.green_on_black()}Active gateway workers: #{length(gateway_pids)}#{CF.reset()}")
      gateway_pids |> Enum.each(fn pid ->
        IO.puts("  • #{inspect(pid)}")
      end)
    rescue
      e -> IO.puts("#{CF.red_on_black()}❌ Failed to get cluster status: #{inspect(e)}#{CF.reset()}")
    end
  end
  ...
  defp exit_console do
    ...
    # Clean up observers
    observers = :ets.tab2list(@observer_registry)
    observers |> Enum.each(fn {name, {_type, _selector, pid}} ->
      if Process.alive?(pid) do
        Process.exit(pid, :kill)
